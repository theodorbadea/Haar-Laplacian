...
log_path = './tmp/res/Node_MagNet_' + dataset_name
...
L = utils.hermitian_decomp_sparse(f_node, e_node, size, gcn_appr=True, edge_weight=dataset.edge_weight)
...
for i in range(len(L)):
    L_img.append(utils.sparse_mx_to_torch_sparse_tensor(L[i].imag))
    L_real.append(utils.sparse_mx_to_torch_sparse_tensor(L[i].real))
    ...
    for num_filter in [16, 32, 64]:
        for layer in [2, 4, 8]:
            ...
            for i in range(splits):
                ...
                model = ChebNet(in_c=X_real.size(-1), L_norm_real=L_real, L_norm_imag=L_img, \
