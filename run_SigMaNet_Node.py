...
from sigmanet import SigMaNet_node_prediction_one_laplacian
...
log_path = './tmp/res/Node_SigMaNet_' + dataset_name
...
edge_index, norm_real, norm_imag = utils.process_magnetic_laplacian(edge_index=dataset.edge_index, gcn=True, net_flow=True,\
                                                    x_real=X_real, edge_weight=dataset.edge_weight, normalization='sym', return_lambda_max=False)
    ...
    for num_filter in [16, 32, 64]:
        for layer in [2, 4, 8]:
            ...
            for i in range(splits):
                ...
                model = SigMaNet_node_prediction_one_laplacian(K=1, num_features=X_real.size(-1), hidden=num_filter, label_dim=label_dim, dropout=dropout,\
...
for i in range(10):
    ...
    model = SigMaNet_node_prediction_one_laplacian(K=1, num_features=X_real.size(-1), hidden=best_error_model_num_filter, label_dim=label_dim, dropout=dropout,\
    ...
    model = SigMaNet_node_prediction_one_laplacian(K=1, num_features=X_real.size(-1), hidden=best_acc_model_num_filter, label_dim=label_dim, dropout=dropout,\
